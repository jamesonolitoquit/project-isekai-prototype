...
function getEmotionalColor(emotionalState?: any): string {
  ...
  const avgEmotion = (
    ...
    (100 - emotionalState.fear) + // Invert fear for positivity
    emotionalState.gratitude -
    emotionalState.resentment
  ) / 4;

  if (avgEmotion > 70) return '#4ade80'; // Green - Very positive
  if (avgEmotion > 50) return '#60a5fa'; // Blue - Positive
  if (avgEmotion > 30) return '#fbbf24'; // Amber - Neutral
...
function DialogueEntry({ entry }: { entry: ParsedDialogueEntry }): React.ReactElement {
  ...
  return (
    <div className="dialog-entry" style={{ marginBottom: '16px', paddingBottom: '12px', borderBottom: '1px solid #333' }}>
      {/* NPC Name with emotional indicator */}
...
export default function DialogPanel({ state, onChoose, isDeveloperMode = false }: DialogPanelProps) {
  ...
  // Parse dialogue entries to extract stage directions and emotional state
