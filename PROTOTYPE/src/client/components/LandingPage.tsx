...
export const LandingPage: React.FC<LandingPageProps> = ({ onEnter, isLoading = false }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!gl.getShaderParameter(vertexShader, gl.COMPILE_STATUS)) {
        console.error('[LandingPage] Vertex shader compilation failed:', gl.getShaderInfoLog(vertexShader));
        return;
      }
      if (!gl.getShaderParameter(fragmentShader, gl.COMPILE_STATUS)) {
        console.error('[LandingPage] Fragment shader compilation failed:', gl.getShaderInfoLog(fragmentShader));
        return;
      }
      ...
      if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
        console.error('[LandingPage] Shader program linking failed:', gl.getProgramInfoLog(program));
        return;
      }
  ...
  const handleConfirmCharacter = () => {
    console.log('[LandingPage] handleConfirmCharacter called', {
      characterName,
      selectedArchetype,
      pageState,
      isLoading
    ...
    if (!characterName || !selectedArchetype) {
      console.warn('[LandingPage] Missing character name or archetype', {
        characterName,
        selectedArchetype
      });
    ...
    console.log('[LandingPage] Setting pageState to complete and calling onEnter');
    setPageState('complete');
    const timer = setTimeout(() => {
      console.log('[LandingPage] Timeout finished, calling onEnter with', { characterName, selectedArchetype });
      try {
        onEnter(characterName, selectedArchetype);
        console.log('[LandingPage] onEnter completed successfully');
      } catch (error) {
        console.error('[LandingPage] Error in onEnter:', error);
        // Reset state if onEnter fails
        setPageState('vessel_confirmation');
      }
    }, 1500);
    
    console.log('[LandingPage] Scheduled onEnter in 1500ms, timer id:', timer);
  };
