...
/**
 * M35 Task 2: Era-Adaptive Glitch Map
 * 
 * Renders epoch-specific visual corruption and decay on the map.
 * Each era (Epoch I, II, III) has unique glitch patterns and intensities.
 * 
 * Epoch I (Awakening): Minimal glitch, clear visuals
 * Epoch II (Unfolding): Moderate glitch, temporal echoes
 * Epoch III (Convergence): Heavy glitch, reality breaks visible
 */

export interface GlitchMapLayer {
  id: string;
  epoch: number;  // 1, 2, or 3
  glitchIntensity: number;  // 0-100
  corruptionPatches: CorruptionPatch[];
  renderMode: 'clear' | 'temporal' | 'shattered';
  overlayOpacity: number;  // 0-1
  scanlineFrequency: number;  // Hz for visual scan effect
}

export interface CorruptionPatch {
  id: string;
  x: number;
  y: number;
  radius: number;
  corruptionLevel: number;  // 0-100
  biomeAffected: string;
  glitchType: 'pixelate' | 'chromatic' | 'timefold' | 'void';
}

/**
 * Calculate glitch intensity based on epoch
 * Epoch I: 0-20%, Epoch II: 20-60%, Epoch III: 60-100%
 */
export function getEpochGlitchIntensity(
  epoch: number,
  contentMutations: number = 0,
  temporalParadoxes: number = 0
): number {
  const baseIntensity = {
    1: 10,   // Epoch I: minimal
    2: 40,   // Epoch II: moderate
    3: 75,   // Epoch III: heavy
  }[epoch] ?? 50;

  // Mutations add 0.5% per mutation (up to +15%)
  const mutationBonus = Math.min(contentMutations * 0.5, 15);

  // Paradoxes add 1% per paradox (up to +25%)
  const paradoxBonus = Math.min(temporalParadoxes * 1, 25);

  return Math.min(baseIntensity + mutationBonus + paradoxBonus, 100);
}

/**
 * Determine render mode based on epoch and glitch intensity
 */
export function getRenderMode(epoch: number, glitchIntensity: number): 'clear' | 'temporal' | 'shattered' {
  if (epoch === 1 && glitchIntensity < 20) return 'clear';
  if (epoch <= 2 && glitchIntensity < 60) return 'temporal';
  return 'shattered';
}

/**
 * Generate glitch map layer for current world state
 */
export function generateGlitchMapLayer(
  state: WorldState,
  epoch: number
): GlitchMapLayer {
  ...
  return {
    id: `glitch_epoch${epoch}_${Date.now()}`,
    epoch,
    glitchIntensity,
    corruptionPatches,
    renderMode,
    overlayOpacity: Math.min(glitchIntensity / 100, 0.8),
    scanlineFrequency
  };
}

/**
 * Apply glitch distortion to viewport coordinates
 * Creates the visual "wobble" effect on render
 */
export function applyGlitchToViewport(
  viewport: MapViewport,
  glitchLayer: GlitchMapLayer,
  time: number
): MapViewport {
  if (glitchLayer.glitchIntensity < 10) {
    return viewport; // No glitch effect needed
  }

  // Sine wave distortion based on time for animated effect
  const distortionAmount = (glitchLayer.glitchIntensity / 100) * 20;
  const wobble = Math.sin(time * 0.002 + glitchLayer.epoch) * distortionAmount;
  const shimmer = Math.cos(time * 0.003) * (distortionAmount * 0.5);

  return {
    minX: viewport.minX + wobble,
    maxX: viewport.maxX + wobble + shimmer,
    minY: viewport.minY + shimmer,
    maxY: viewport.maxY + shimmer + wobble,
  };
}

/**
 * Get CSS filter string for epoch-specific visual corruption
 * Used for rendering the glitch effect on map canvas/SVG
 */
export function getCorruptionVisuals(glitchLayer: GlitchMapLayer): {
  filter: string;
  colorScheme: string;
  backgroundColor: string;
} {
  const intensity = glitchLayer.glitchIntensity;

  // Render mode affects color grading
  const colorSchemes = {
    clear: 'hue-rotate(0deg) saturate(100%)',
    temporal: 'hue-rotate(15deg) saturate(85%)',
    shattered: 'hue-rotate(-30deg) saturate(50%)',
  };

  // Build composite filter
  let filter = colorSchemes[glitchLayer.renderMode];

  // Add brightness reduction for corrupted states
  if (intensity > 40) {
    filter += ` brightness(${Math.max(85 - intensity * 0.2, 60)}%)`;
  }

  // Add contrast for shattered mode
  if (glitchLayer.renderMode === 'shattered') {
    filter += ` contrast(${Math.min(100 + intensity * 0.5, 150)}%)`;
  }

  // Add blur for heavy glitch
  if (intensity > 70) {
    filter += ` blur(${intensity * 0.05}px)`;
  }

  // Scanline effect via invert + mix-blend simulation
  if (intensity > 30) {
    // Note: actual scanlines rendered via SVG overlay or CSS animation
  }
  ...
  return {
    filter,
    colorScheme: glitchLayer.renderMode,
    backgroundColor: backgroundColors[glitchLayer.renderMode]
  };
}

/**
 * Get visual description of current glitch state (for UI display)
 */
export function getGlitchStatus(glitchLayer: GlitchMapLayer): string {
  if (glitchLayer.glitchIntensity === 0) return '✨ Crystal Clear';
  if (glitchLayer.glitchIntensity < 25) return '🌫️ Slight Haze';
  if (glitchLayer.glitchIntensity < 50) return '⚡ Temporal Echoes';
  if (glitchLayer.glitchIntensity < 75) return '🎆 Heavy Corruption';
