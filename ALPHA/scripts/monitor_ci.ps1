...
while((Get-Date) -lt $endTime) {
  ...
  if ($hasChecks) {
    $outDir = Join-Path -Path (Get-Location) -ChildPath 'ALPHA' -AdditionalChildPath 'artifacts'
    ...
    Write-Output "Found CI contexts for commit $sha. Saved to ALPHA/artifacts/ci_check_runs.json and ALPHA/artifacts/ci_status.json"
    exit 0
  }
