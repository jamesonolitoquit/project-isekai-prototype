...
// ALPHA_Audit: Type interfaces for full type safety in faction logic
...
function checkBoredomAndStimulus(
  ...
  if (timeSinceLastEvent >= thresholds.boredomThreshold) {
    // Get player analytics to determine playstyle
    const playerAnalytics = (state.player as any).analytics || [];
    const playstyleVector = (state.player as any).playstyleVector || { 
      combatant: 0.33, 
      diplomat: 0.33, 
      explorer: 0.34,
      dominant: 'hybrid'
...
function updateDirectorZones(state: WorldState, directorState: DirectorState): DirectorZone[] {
  ...
  const currentTick = state.tick ?? 0;
  const playerX = (state.player as any).x ?? 500;
  const playerY = (state.player as any).y ?? 500;
    ...
    if (shouldRelicRebel(relic, paradoxLevel)) {
      ...
      return {
        ...
        payload: {
          ...
          paradoxLevel,
          rebellionCounter: (relic as any).rebellionCounter || 0,
        },
        mutationClass: 'SYSTEM',
        timestamp: Date.now(),
      };
...
function checkPlaystyleAdaptation(
  ...
  (state.player as any).playstyleVector = vector;
...
function checkFactionWorldEvents(
  ...
  const conflicts: FactionConflict[] = (state as any).factionConflicts || [];
  const factions: Faction[] = (state as any).factions || [];
...
function checkNarrativeResonance(
  ...
  const artifactCorruption = (state.relics && Object.values(state.relics).some(r => (r as any).corruption > 70));
...
export function suggestPacingIntervention(state: WorldState): Event | null {
  const currentTick = state.tick ?? 0;
  const playerAnalytics = (state.player as any).analytics || [];
  const inactionCounter = (state.player as any).inactionCounter ?? 0;
  const temporalDebt = (state.player as any).temporalDebt ?? 0;
  ...
  if (interventionType === 'weather_shift') {
    ...
    eventDescription = eventPayload.whisper;
...
export function calculateAdaptiveDifficulty(
  ...
  if (playstyleVector.dominant === 'combatant' && playstyleVector.combatant > 0.7) {
    ...
    const combatHistory = (state.player as any).recentCombatOutcomes || [];
  ...
  if (playstyleVector.dominant === 'diplomat' && playstyleVector.diplomat > 0.7) {
    const socialHistory = (state.player as any).recentDialogueOutcomes || [];
  ...
  if (playstyleVector.dominant === 'explorer' && playstyleVector.explorer > 0.7) {
    const discoveryRate = (state.player as any).locationsDiscoveredRecently || [];
