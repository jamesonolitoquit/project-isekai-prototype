...
export function addPeerToSession(
  ...
  const presence: PeerPresence = {
    ...
    playerName: ((playerState as any).name) || 'Unknown Player',
    status: 'active',
...
export function applyLatencyToUpdate(
  ...
  if ((netSim as any).config?.enabled) {
    // Simulate packet loss
    if (Math.random() < (netSim as any).config.packetLossRate) {
      packet.retransmitCount++;
      return { delayMs: ((netSim as any).avgLatency || 0), shouldApply: false };
    }

    // Simulate latency
    const latency = packet.expectedLocalLatency + (Math.random() - 0.5) * 2 * ((netSim as any).config.latencyVariance || 0);
    return {
...
export function broadcastStateUpdate(
  ...
  for (const peerId of registry.activePlayers) {
    ...
    const expectedLatency = consensusState.networkSimState
      ? (consensusState.networkSimState as any).avgLatency || consensusState.networkSimState.latency || 35
...
export function getConsensusDiagnostics(consensusState: ConsensusState): {
  ...
  const avgLatency = consensusState.networkSimState
    ? ((consensusState.networkSimState as any).avgLatency || consensusState.networkSimState.latency || 0)
    : 0;
