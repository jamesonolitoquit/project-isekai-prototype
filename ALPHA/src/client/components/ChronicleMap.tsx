...
export const ChronicleMap: React.FC<ChronicleMapProps> = ({
  ...
  const fragments = useMemo(() => {
    const allFragments = (worldState as any).worldFragments || [];
    return Array.isArray(allFragments) ? allFragments : [];
  }, [(worldState as any).worldFragments]);
  ...
  const mapDimensions = useMemo(() => {
    ...
    const maxX = Math.max(...locations.map(l => (l as any).x || 0), 100);
    const maxY = Math.max(...locations.map(l => (l as any).y || 0), 100);
    return {
    ...
    <div style={{
      ...
      <div style={{
        ...
        {locations.map((location: Location) => {
          ...
          const x = useAbsolutePositioning ? (location as any).x || Math.random() * 300 : undefined;
          const y = useAbsolutePositioning ? (location as any).y || Math.random() * 300 : undefined;
          ...
          return (
            ...
            >
              ...
              {false && (location as any).biome === 'corrupted' && (
                <div style={{
                  marginTop: '6px',
                  fontSize: '11px',
                  color: '#ff6b6b',
