...
export function useGlobalVisuals(
  ...
  useEffect(() => {
    ...
    if (!latestEvent || lastProcessedEventIdRef.current === latestEvent.id) return;

    // Mark this event as processed
    lastProcessedEventIdRef.current = latestEvent.id;

    if (debug) {
      console.log('[useGlobalVisuals] Event detected:', latestEvent.type);
    }

    // CRITICAL_HIT → Screen shake for 500ms
    if (latestEvent.type === 'CRITICAL_HIT' && !prefersReducedMotion) {
      const impactMagnitude = latestEvent.payload?.impactMagnitude || 1;
    ...
    if (latestEvent.type === 'CAST_SPELL' && !prefersReducedMotion) {
      const visualElement = latestEvent.payload?.visualElement || 'arcane';
    ...
    if (latestEvent.type === 'TEMPORAL_ANOMALY' && !prefersReducedMotion) {
      if (debug) {
    ...
    if (latestEvent.type === 'PARADOX_GLITCH' && !prefersReducedMotion) {
      const glitchIntensity = latestEvent.payload?.intensity || 0.5;
      if (debug) {
