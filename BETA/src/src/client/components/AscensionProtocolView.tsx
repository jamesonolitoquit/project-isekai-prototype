  ...
  return (
    <div style={styles.panel}>
      ...
      <div style={styles.lineageTree}>
        ...
        {bloodlineData && (bloodlineData as any).legacyImpacts && (bloodlineData as any).legacyImpacts.length > 0 && (
          <div style={{ marginTop: '20px', paddingLeft: '20px', borderLeft: '2px solid #4a5568' }}>
            ...
            {(bloodlineData as any).legacyImpacts.slice(-3).reverse().map((ancestor: any, idx: number) => (
  ...
  const renderPanel = () => {
    switch (panelState.activePanel) {
      ...
      case 'heirloom':
        return (
          <HeirloomVaultPanel
            items={(character.inventory || []).map((item: any) => ({
