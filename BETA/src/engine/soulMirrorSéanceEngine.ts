 ...
 * Phase 13: Enhanced to unlock clues in investigation pipeline
 */
export function queryAncestorGuidance(
  state: WorldState,
  playerResonanceLevel: number,
  query: SéanceQuery,
  investigationPipeline?: any  // Phase 13: Optional investigation pipeline for clue unlocking
): SéanceResult {
  const events: Event[] = [];
  const ancestorGuidances: AncestorGuidance[] = [];
  const markerLocations: string[] = [];
  const unlockedClueIds: string[] = [];
  let resonanceCost = 0;
  ...
  if (query.investigationChainId) {
    ...
    if (guidance) {
      ...
      if (guidance.clueIds) {
        ...
        unlockedClueIds.push(...guidance.clueIds);
      }
    }
  }
  ...
  if (guidanceCount < maxGuidances && query.factionId) {
    ...
    if (factionGuidance) {
      ...
      if (factionGuidance.clueIds) {
        unlockedClueIds.push(...factionGuidance.clueIds);
      }
    }
  }

  // === Phase 13 Task 1 Integration ===
  // Unlock clues in investigation pipeline if provided
  if (investigationPipeline && unlockedClueIds.length > 0) {
    for (const clueId of unlockedClueIds) {
      try {
        // Mark clue as discovered in player knowledge base
        if (state.player && state.player.knowledgeBase) {
          if (state.player.knowledgeBase instanceof Map) {
            state.player.knowledgeBase.set(`clue:${clueId}`, { type: 'clue', id: clueId, unlockedVia: 'ancestor_guidance' });
          } else if (Array.isArray(state.player.knowledgeBase)) {
            state.player.knowledgeBase.push(`clue:${clueId}`);
          }
        }

        // Emit ANCESTOR_CLUE_REVEALED event for each unlocked clue
        const clueRevealEvent: Event = {
          id: `ancestor-clue-revealed-${clueId}-${Date.now()}`,
          worldInstanceId: state.id,
          actorId: 'player',
          type: 'ANCESTOR_CLUE_REVEALED',
          payload: {
            clueId,
            unlockedBy: ancestorGuidances[0]?.ancestorName || 'Unknown Ancestor',
            resonanceStrength: playerResonanceLevel
          },
          timestamp: Date.now()
        };
        events.push(clueRevealEvent);
      } catch (err) {
        // Non-fatal: clue unlock is advisory
        if (process.env.NODE_ENV === 'development') {
          console.warn(`[SoulMirrorSéance] Failed to unlock clue ${clueId}:`, err);
        }
      }
    }
  }

  // Emit main séance event
  const séanceEvent: Event = {
    ...
    payload: {
      ...
      resonanceCost,
      guidanceCount,
      cluesUnlocked: unlockedClueIds.length,  // Phase 13: Track clues unlocked
  ...
  for (const guidance of ancestorGuidances) {
    const revelationEvent: Event = {
      ...
      payload: {
        ...
        cluesUnlocked: guidance.clueIds?.length || 0  // Phase 13: Include clue count
      },
      timestamp: Date.now()
    };
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    if (unlockedClueIds.length > 0) {
      console.log(`[SoulMirrorSéance] Phase 13: Unlocked ${unlockedClueIds.length} clues via ancestor guidance`);
    }
  }
