...
/**
 * Phase 13 Task 3: Check for Temporal Fractures
 * When generationalParadox reaches critical thresholds, reality warping anomalies manifest
 * @param generationalParadox - Cumulative paradox across all epochs (never resets)
 * @param state - Current WorldState for context
 * @returns Object describing fracture manifestation
 */
export interface TemporalFractureResult {
  ...
  anomalyType?: 
    ...
    | 'echo_conflict'      // Contradictory ancestor guidance appears
    | 'timeline_stutter'   // NPCs repeat actions from previous epoch
    | 'item_duplication';  // Inventory items spontaneously duplicate
  severity?: 'minor' | 'major' | 'catastrophic';
  affectedEntityId?: string;  // NPC ID, location ID, or item ID affected
  manifestationText?: string; // Narrative description of anomaly
}

export function checkTemporalFractures(
  generationalParadox: number,
  state?: WorldState
): TemporalFractureResult {
  // No fractures below 150 cumulative paradox
  if (generationalParadox < 150) {
    return { hasFractures: false };
  }

  // Determine severity based on paradox magnitude
  let severity: 'minor' | 'major' | 'catastrophic' = 'minor';
  if (generationalParadox >= 300) {
    severity = 'catastrophic';
  } else if (generationalParadox >= 225) {
    severity = 'major';
  }

  // Select anomaly type based on severity and random seed
  const anomalyChance = Math.min(0.95, (generationalParadox - 150) / 150);
  if (random() > anomalyChance) {
    return { hasFractures: false };
  }

  const anomalyTypes: TemporalFractureResult['anomalyType'][] = [
    'phased_npc',
    'weather_loop',
    'location_drift',
    'echo_conflict',
  ...
  const anomalyType = anomalyTypes[Math.floor(random() * anomalyTypes.length)];
  let affectedEntityId: string | undefined;
    ...
    switch (anomalyType) {
      case 'phased_npc': {
        if (state.npcs.length > 0) {
          const npc = state.npcs[Math.floor(random() * state.npcs.length)];
          affectedEntityId = npc.id;
          manifestationText = `${npc.name} flickers between two forms—briefly appearing in two places at once before reality snaps back into focus.`;
        }
        break;
      }
      case 'weather_loop': {
        manifestationText = `The sky cycles through the same weather pattern repeatedly—as if time itself is echoing.`;
        break;
      }
      case 'location_drift': {
        if (state.locations.length > 0) {
          const loc = state.locations[Math.floor(random() * state.locations.length)];
          affectedEntityId = loc.id;
          manifestationText = `${loc.name}'s coordinates seem to shift when you're not looking directly at it.`;
        }
        break;
      }
      case 'echo_conflict': {
        manifestationText = `Ancestor voices speak contradictory guidance—one urges caution, another demands boldness. Their counsel conflicts across timelines.`;
        break;
      }
      case 'timeline_stutter': {
        if (state.npcs.length > 0) {
          const npc = state.npcs[Math.floor(random() * state.npcs.length)];
          affectedEntityId = npc.id;
          manifestationText = `${npc.name} repeats the same dialogue—as if trapped in yesterday's conversation loop.`;
        }
        break;
      }
      case 'item_duplication': {
        if (state.player?.inventory && state.player.inventory.length > 0) {
          ...
          affectedEntityId = 'inventory';
          const itemName = 'kind' in item && item.kind === 'unique' ? item.itemId : item.itemId;
          manifestationText = `Your ${itemName} briefly flickers—for a moment, you could swear you held two copies.`;
        }
        break;
      }
    }
  ...
  return {
    ...
    severity,
    affectedEntityId,
    manifestationText: manifestationText || 'Reality warps briefly—the world remembers multiple timelines at once.'
  };
}
