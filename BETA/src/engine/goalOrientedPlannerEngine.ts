...
export interface NpcGoal {
  id: string;
  goalType: 'explore' | 'gather' | 'socialize' | 'combat' | 'flee' | 'rest' | 'trading_rumors' | 'trade' | 'custom';
  priority: number;
...
class GoalOrientedPlanner {
  ...
  private getApplicableActions(npc: NPC, goal: NpcGoal, worldState: WorldState): NpcAction[] {
    ...
    switch (goal.goalType) {
      ...
      case 'trading_rumors':
        ...
        break;
      case 'trade':
        // Beta Phase 5 Task 4: Trade harvested resources with merchants
        actions.push(this.createTradeAction(goal.targetNpcId || ''));
        break;
      case 'combat':
  ...
  private createTradeAction(targetNpcId: string): NpcAction {
    // Beta Phase 5 Task 4: Trade harvested regional resources with merchant NPCs
    return {
      id: `action_trade_${targetNpcId}`,
      actionType: 'trade',
      preconditions: { isAlive: true, hasInventory: true, canTrade: true },
      effects: { inventoryChanged: true, goldChanged: true, tradeComplete: true },
      cost: 8,
      targetNpcId: targetNpcId,
      duration: this.rng.nextInt(30, 90),
      parameters: { tradeType: 'regional_resources', maxItemsToTrade: 5 }
    };
  }
