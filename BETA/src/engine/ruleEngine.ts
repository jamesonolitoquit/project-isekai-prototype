...
export function resolveCombat(
  ...
  // ALPHA_M15: Check if this is a critical hit
  const critRoll = rollCriticalStrike(attackerStats);
  const isCritical = critRoll.critical;
  const finalDamage = isCritical ? Math.floor(damage * critRoll.multiplier) : damage;
  const impactMagnitude = finalDamage / 10; // Scale impact intensity with damage (0.1-3.0+)
  
  logs.push(`Damage dealt: ${finalDamage} HP.${isCritical ? ` CRITICAL x${critRoll.multiplier.toFixed(1)}!` : ''}`);

  // ALPHA_M15: Emit CRITICAL_HIT event if applicable
  if (isCritical) {
    events.push({
      type: 'CRITICAL_HIT',
      payload: {
        ...
        defenderId: primaryDefender,
        baseDamage: damage,
        criticalMultiplier: critRoll.multiplier,
  ...
  // ALPHA_M15: Emit COMBAT_HIT with impact magnitude for particle scaling
  events.push({
    type: 'COMBAT_HIT',
    payload: {
      attackerId,
      defenderId: primaryDefender,
      damage: finalDamage,
      impactMagnitude, // Scales particle intensity with damage
      isCritical,
      logs
    },
    id: crypto.randomUUID(),
