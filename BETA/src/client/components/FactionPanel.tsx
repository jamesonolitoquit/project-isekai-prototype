...
import type { Faction, FactionRelationship } from '../../engine/factionEngine';
...
export default function FactionPanel({ 
  ...
  const worldTension = useMemo(() => {
    try {
      ...
      if (error instanceof Error) {
        console.warn('Failed to calculate world tension:', error.message);
      }
      return 0;
    }
  }, [state]);
  
  // Helper: Determine attitude from reputation score
  const getAttitudeFromReputation = (playerRep: number): 'allied' | 'neutral' | 'hostile' => {
    if (playerRep >= 50) return 'allied';
    if (playerRep <= -50) return 'hostile';
    return 'neutral';
  };
  
  // Convert factions to display format
  const displayFactions: DisplayFaction[] = factions.map((faction: Faction) => {
    const playerRep = factionReputation[faction.id] || 0;
    const attitude = getAttitudeFromReputation(playerRep);
  ...
  const getRelationships = (factionId: string): DisplayRelationship[] => {
    return factionRelationships
      .filter((rel: FactionRelationship) => rel.factionAId === factionId || rel.factionBId === factionId)
      .map((rel: FactionRelationship) => ({
  ...
  const getRecentEvents = (): string[] => {
    ...
    // ALPHA_M3: Use real faction events from metadata
    const factionEvents = state?.metadata?.factionEvents || [];
    
    // Display recent faction events
    const recentFactionEvents = factionEvents.slice(-5).reverse();
    for (const event of recentFactionEvents) {
      switch (event.type) {
        case 'power-shift':
          if (event.delta > 0) {
            events.push(`📈 ${event.factionName} gains power (+${event.delta})`);
          } else if (event.delta < 0) {
            events.push(`📉 ${event.factionName} loses power (${event.delta})`);
          }
          break;
        case 'conflict-started':
          events.push(`⚔️ War! ${event.factionA} conflicts with ${event.factionB}`);
          break;
        case 'conflict-resolved':
          events.push(`✓ ${event.winner} defeats ${event.loser} in ${event.conflictType}`);
          break;
      }
    }

    // Also add reputation-based events
    displayFactions.forEach(faction => {
      if (faction.attitude === 'allied' && faction.playerReputation >= 80) {
        events.push(`✓ ${faction.name} considers you an ally`);
      } else if (faction.attitude === 'hostile' && faction.playerReputation <= -80) {
        events.push(`✗ ${faction.name} will attack on sight`);
      }
    });
