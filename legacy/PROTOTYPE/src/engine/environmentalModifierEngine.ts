...
export function generateFluxAnomaly(
  ...
  const selectedType = rng.pick ? rng.pick(anomalyTypes) : anomalyTypes[Math.floor((rng.next?.() ?? 0.5) * anomalyTypes.length)];
  const selectedIntensity = rng.pick ? rng.pick(intensities) : intensities[Math.floor((rng.next?.() ?? 0.5) * intensities.length)];

  const durationBase = 60; // Ticks
  const duration = durationBase + (rng.nextInt ? rng.nextInt(0, 60) : Math.floor((rng.next?.() ?? 0.5) * 60));

  const manifestations: Record<FluxAnomaly['type'], Record<string, string>> = {
    temporal_echo: {
  ...
  const mechanicalEffects: Record<FluxAnomaly['type'], Record<string, string>> = {
    temporal_echo: {
  ...
  const manifestation = manifestations[selectedType][selectedIntensity];
  const mechanicalEffect = mechanicalEffects[selectedType][selectedIntensity];
